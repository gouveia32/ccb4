...
namespace DAL
{
    public class DALItem
    {
        ...
        private Item BancoParaModelo(MySqlDataReader registro)
        {
            ...
            if (!DBNull.Value.Equals(registro["bordado_id"]))  // Existe bordado associado
            {
        ...
        public DataTable ItensDoPedido(int pedido_id)
        {
            ...
            p = new List<MySqlParametro>();
            p.Add(new MySqlParametro("@pedido_id", pedido_id));
            sql = "SELECT pedido_id, clientes.nome, clientes.endereco,item, itens.descricao, data_entrega, pc_solicitadas, preco_por_peca,  pc_solicitadas * preco_por_peca AS total, bordado_id, local_id,lado,itens.obs FROM itens LEFT JOIN pedidos ON pedidos.id=itens.pedido_id JOIN clientes ON pedidos.cliente_id=clientes.id WHERE pedido_id = @pedido_id;";
