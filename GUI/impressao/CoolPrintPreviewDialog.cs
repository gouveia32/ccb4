    ...
    internal partial class CoolPrintPreviewDialog : DevExpress.XtraEditors.XtraForm
    {
        ...
        void _btnPrint_Click(object sender, EventArgs e)
        {
            using (var dlg = new PrintDialog())
            {
                ...
                {
                    // print selected page range
                    dlg.PrinterSettings.PrinterName = cbImpressoras.Text;
                    _preview.Print();
                }
        ...
        private void CoolPrintPreviewDialog_Load(object sender, EventArgs e)
        {
            ...
            //inicia com Zoom: tela preenchida
            _preview.ZoomMode = _preview.ZoomMode == ZoomMode.ActualSize
                ? ZoomMode.FullPage
                : ZoomMode.ActualSize;
            ...
            {
                cbImpressoras.Items.Add(printname);
            }

            //Carrega a última impressora utilizada
            Microsoft.Win32.RegistryKey key = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("Software\\ccb4");
            if (key != null)
               try
                {
                    cbImpressoras.Text = key.GetValue("Impressora", "").ToString();
                }
                finally
                {
                    key.Close();
        ...
        private void cbImpressoras_SelectedIndexChanged(object sender, EventArgs e)
        {
            Microsoft.Win32.RegistryKey key = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("Software\\ccb4");
            try
            {
                key.SetValue("Impressora", cbImpressoras.Text);
            }
            finally
            {
                key.Close();
