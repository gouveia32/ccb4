...
namespace GUI
{
    public partial class frmAgendaCriacao : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmAgendaCriacao(frmPrincipal _fp)
        {
            InitializeComponent();
            fp = _fp;
        }
        ...
        private static void SetupLabels(ISchedulerStorage storage)
        {
            ...
            label = storage.Appointments.Labels.CreateNewLabel("Normal");
            label.Color = Color.FromArgb(BLL.Properties.Settings.Default.corCriacaoNormal);
            storage.Appointments.Labels.Add(label);

            label = storage.Appointments.Labels.CreateNewLabel("Urgente");
            label.Color = Color.FromArgb(BLL.Properties.Settings.Default.corCriacaoUrgente);
            storage.Appointments.Labels.Add(label);

            label = storage.Appointments.Labels.CreateNewLabel("Compromisso");
        ...
        private void frmAgendaCriacao_Load(object sender, EventArgs e)
        {
            inicioSemana = DateTime.Now.AddDays(-(int)(DateTime.Now.DayOfWeek + 7));

            SetupLabels(SchedulerStorage1);

            //Timer1.Interval = frmPreferencias.intTempoAtualizaCriacaos.Value * 1000;
